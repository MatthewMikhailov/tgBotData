...
def main(filename, chat_id, max_alowed_process):
    ...
    with Manager() as manager:
        ...
        for task_id in range(len(prompts)):
            ...
            Process(target=get_data, args=(prompts[task_id], m_dict, chat_id), name=str(len(active_children()) - 1)).start()
...
def run_parser_wb(filename, chat_id, max_alowed_process):
    try:
...
if __name__ == '__main__':
    print(return_time_wb('ТестMP.xlsx', 4) // 60)
    run_parser_wb('ТестMP.xlsx', 1275943662, 4)
